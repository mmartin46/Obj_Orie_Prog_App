...
class MediatorScreen(Screen):
   ...
   def on_enter(self, *args):
      ...
      self.title_label.text=self.db.get_query("""SELECT p_name FROM all_patterns WHERE p_name LIKE 'Mediator%'""")
   ...
   #   self.main_lout.spacing=3
      ...
      self.desc_label.text = str(GetDesignDesc('Mediator').java_design_desc())
      ...
      self.img = Image(source='code_imgs\\mediator_0.PNG')
      self.img.pos_hint={'center_x': 0.5, 'center_y': 0.86}
      self.img.size_hint_y=.6
      self.img.size_hint_x=.6
      self.bk_drop.add_widget(self.img)
            
      self.img_2 = Image(source='code_imgs\\mediator_1.PNG')
      self.img_2.pos_hint={'center_x': 0.5, 'center_y': 0.75}
